...
ActorPlaneFlapLeft::ActorPlaneFlapLeft(ActorManager*  pActorManager) : ActorHierarchy(pActorManager)
{
	name_ = ActorManager::NAME_PLANE_FLAP_LEFT;
	tag_ = ActorManager::TAG_PLANE;
	layer_ = ActorManager::LAYER_PLANE;
}

ActorPlaneFlapLeft::~ActorPlaneFlapLeft()
{

}

bool ActorPlaneFlapLeft::Init()
{
	nameUnique_ = "ActorPlaneFlapLeft" + std::to_string(number_);
	...
	transOffset_ = { -126.0f, 22.0f, -305.0f };
...
void ActorPlaneFlapLeft::Uninit()
{

}

void ActorPlaneFlapLeft::Update()
{
	ReflectionMatrix();
}

void ActorPlaneFlapLeft::Stats()
{
	ActorHierarchy::Stats();
}

void ActorPlaneFlapLeft::Draw()
{
	{
		//コンスタントバッファ作成
	...
	//モデル描画
	pModelManager_->GetInstance(ModelManager::HORNET_FLAP_LEFT)->Draw();
}
