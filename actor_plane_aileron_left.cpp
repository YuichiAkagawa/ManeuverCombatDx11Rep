...
ActorPlaneAileronLeft::ActorPlaneAileronLeft(ActorManager*  pActorManager) : ActorHierarchy(pActorManager)
{
	name_ = ActorManager::NAME_PLANE_AILERON_LEFT;
	tag_ = ActorManager::TAG_PLANE;
	layer_ = ActorManager::LAYER_PLANE;
}

ActorPlaneAileronLeft::~ActorPlaneAileronLeft()
{

}

bool ActorPlaneAileronLeft::Init()
{
	nameUnique_ = "ActorPlaneAileronLeft" + std::to_string(number_);
	...
	transOffset_ = { -423.5f, 11.7f, -349.0f };
...
void ActorPlaneAileronLeft::Uninit()
{

}

void ActorPlaneAileronLeft::Update()
{
	ReflectionMatrix();
}

void ActorPlaneAileronLeft::Stats()
{
	ActorHierarchy::Stats();
}

void ActorPlaneAileronLeft::Draw()
{
	{
		//コンスタントバッファ作成
	...
	//モデル描画
	pModelManager_->GetInstance(ModelManager::HORNET_AILERON_LEFT)->Draw();
}
